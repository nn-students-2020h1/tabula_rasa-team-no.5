...
@mylogs
def last_message(update: Update, context: CallbackContext):
    """Send a last message when the command /last_message is issued"""
    i = 2
    while update.effective_user.first_name != loglist[-i]['user']:
        i += 1
    update.message.reply_text(f'Твоё последнее сообщение было:"{loglist[-i]["message"]}"')
...
def main():
    ...
    updater.dispatcher.add_handler(CommandHandler('last_message', last_message))
